...
class LetterOfIntroductionReport {
  static Future<Uint8List> build({
    ...
    final List<Map<String, dynamic>> conditionRows = await DatabaseManager()
        .getConditionHistoryRows(patient.patientUuid);
    final List<Map<String, dynamic>> medicationRows = await DatabaseManager()
        .getActiveMedicationRows(patient.patientUuid);
    final List<Map<String, dynamic>> allergyRows = await DatabaseManager()
        .getAllergyDetailRows(patient.patientUuid);
    final List<Map<String, dynamic>> symptomRows = await DatabaseManager()
        .getActiveSymptomRows(patient.patientUuid);

    final List<String> conditionLines = conditionRows.map((row) {
      final ConditionStatus status =
          ConditionStatus.values[row['status'] as int? ?? 0];
      final String onset = row['onset'] != null
          ? ReportPdfBuilder.formatDate(DateTime.parse(row['onset'] as String))
          : 'date unknown';
    ...
    final List<String> allergyLines = allergyRows.map((row) {
      final AllergySeverity severity =
          AllergySeverity.values[row['severity'] as int? ?? 0];
    ...
    final List<String> concernLines = symptomRows.map((row) {
      ...
      final String severity = severityIndex != null
          ...
          : '';
      final String frequency = frequencyIndex != null
          ? Frequency.values[frequencyIndex].label
          : '';
